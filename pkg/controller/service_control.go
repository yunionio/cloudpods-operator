...
func (c *realServiceControl) CreateService(oc *v1alpha1.OnecloudCluster, svc *corev1.Service) error {
	_, err := c.kubeCli.CoreV1().Services(oc.Namespace).Create(context.Background(), svc, v1.CreateOptions{})
...
func (c *realServiceControl) UpdateService(oc *v1alpha1.OnecloudCluster, svc *corev1.Service) (*corev1.Service, error) {
	...
	err := retry.RetryOnConflict(retry.DefaultBackoff, func() error {
		var updateErr error
		updateSvc, updateErr = c.kubeCli.CoreV1().Services(ns).Update(context.Background(), svc, v1.UpdateOptions{})
		if updateErr == nil {
...
func (c *realServiceControl) DeleteService(oc *v1alpha1.OnecloudCluster, svc *corev1.Service) error {
	err := c.kubeCli.CoreV1().Services(oc.Namespace).Delete(context.Background(), svc.Name, v1.DeleteOptions{})
