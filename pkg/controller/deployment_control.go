...
func (c *realDeploymentControl) CreateDeployment(oc *v1alpha1.OnecloudCluster, deploy *apps.Deployment) error {
	_, err := c.kubeCli.AppsV1().Deployments(oc.Namespace).Create(context.Background(), deploy, metav1.CreateOptions{})
...
func (c *realDeploymentControl) UpdateDeployment(oc *v1alpha1.OnecloudCluster, deploy *apps.Deployment) (*apps.Deployment, error) {
	...
	err := retry.RetryOnConflict(retry.DefaultBackoff, func() error {
		var updateErr error
		updatedDeploy, updateErr = c.kubeCli.AppsV1().Deployments(ns).Update(context.Background(), deploy, metav1.UpdateOptions{})
		if updateErr == nil {
...
func (c *realDeploymentControl) DeleteDeployment(oc *v1alpha1.OnecloudCluster, deploymentName string) error {
	err := c.kubeCli.AppsV1beta1().Deployments(oc.Namespace).Delete(context.Background(), deploymentName, metav1.DeleteOptions{})
